...
export default function PageOrganizer({
  ...
  onRotate, onDelete, onRestore, onDuplicate, onExtract,
}: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-[#C8CDD2] px-4 py-2.5 text-sm">
        ...
        <ToolBtn icon={Download} label="Extract" disabled={!selIds.length} onClick={() => onExtract(selIds)} />
      ...
      {/* grid */}
      <div className="grid flex-1 grid-cols-[repeat(auto-fill,minmax(140px,1fr))] content-start gap-4 overflow-auto bg-[#F7F9FB] p-4">
        {pages.map((p, i) => {
          const key = `${p.id}:${p.rotation}`;
          ...
          return (
            ...
            >
              ...
              {/* selection badge — visible state for multi-select */}
              <div
                className={clsx(
                  ...
                  isSel
                    ...
                    : "border-[#C8CDD2] bg-white/90 text-transparent group-hover:text-[#C8CDD2]",
                )}
              >
                {isSel ? <CheckSquare className="h-3.5 w-3.5" /> : <Square className="h-3.5 w-3.5" />}
              </div>
              <div className="flex aspect-[3/4] items-center justify-center overflow-hidden rounded bg-[#EEF1F4]">
                {thumbs[key] ? (
...
}: { icon: typeof RotateCw; label: string; onClick: () => void; disabled?: boolean; danger?: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
