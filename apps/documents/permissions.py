...
class HasDocumentPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        from apps.documents.file_streaming import (
            user_is_involved_with_document,
            user_has_signed_document,
            user_has_completed_workflow_task_on_document,
        )
        if not user_is_involved_with_document(request.user, obj):
            ...
            if required_action == GroupAction.VIEW.value and user_has_signed_document(request.user, obj):
                return True
            # Similarly, someone who completed a workflow task on this document keeps
            # permanent VIEW-only access to what they approved/rejected/returned.
            if required_action == GroupAction.VIEW.value and user_has_completed_workflow_task_on_document(request.user, obj):
                return True
            return False
